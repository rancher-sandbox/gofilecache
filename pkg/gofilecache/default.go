...
func DefaultDir() string {
	...
	defaultDirOnce.Do(func() {
		defaultDir = os.Getenv("GOFILECACHE")
		if filepath.IsAbs(defaultDir) || defaultDir == "off" {
			return
		}
		if defaultDir != "" {
			defaultDir = "off"
			defaultDirErr = fmt.Errorf("GOFILECACHE is not an absolute path")
			return
		}
		...
		if err != nil {
			defaultDir = "off"
			defaultDirErr = fmt.Errorf("GOFILECACHE is not defined and %v", err)
			return
		}
